...
async function optimizeImage(file, targetSize) {
  ...
  const optimizedPath = path.join('/tmp/optimized', optimizedFileName);
  const thumbnailPath = path.join('/tmp/thumbnails', thumbnailFileName);

  await fs.mkdir('/tmp/optimized', { recursive: true });
  await fs.mkdir('/tmp/thumbnails', { recursive: true });
